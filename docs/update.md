...
戻したときは、その配布物を覚えておく。覚えないと、開き直した App が次の確認で同じものをまた取得し、次の起動でまた戻すことになる。起動のたびに数十〜百数十 MB を空振りする。覚えるのはタグと SHA-256 の組なので、**同じタグへ直した一式が上げ直されれば取りに行く。** その間、設定カードの「本体の更新」欄には取得しない旨を出す。

起動直後に終わること自体は異常ではない。既に別の App が動いていた場合と、取得しておいた更新をヘルパへ渡した場合は、どちらも正常な終了として扱われる。戻す判断に使うのは、終了コードが 0 以外だった場合だけである。

見ているのはプロセスが落ちたかどうかである。起動の途中で例外が出た場合はアプリごと落ちるので (下記)、ここで拾える。ウィンドウを出した後の描画で失敗するような、起動を抜けてから起こる形は拾えない。
...
ウィンドウを組み立てるところまでの間に例外が出た場合、アプリはそのまま落ちる。握り潰して生き延びさせない。

握り潰すと、ウィンドウの無いプロセスがメッセージループと多重起動の抑止を掴んだまま残る。利用者から見ると、画面が出ないうえに、もう一度起動しても抑止に当たって何も出てこない。タスクマネージャで止めるまで抜けられない。落ちれば抑止は OS が手放すので、原因を取り除いて起動し直せる。

更新の直後であれば、上に書いた更新ヘルパの観察がこれを拾い、退避しておいた一式へ戻す。

ウィンドウを出した後の例外はこれまでどおり握り潰す。常駐中の例外でいちいち落ちては、同期の途中で画面ごと消えることになる。
