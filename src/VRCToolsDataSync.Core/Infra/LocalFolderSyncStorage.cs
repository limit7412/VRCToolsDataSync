...
public sealed class LocalFolderSyncStorage : ISyncStorage
{
    ...
    {
        var path = StorageKey.ToLocalPath(_rootDirectory, ManifestKeys.Manifest);
        if (!File.Exists(path))
        {
    ...
    /// (<c>ManifestStore.UpdateToolEntry</c> の version 検査で、同じ tool に
    ...
    /// <para>
    /// 回収 (<c>BlobGarbageCollector</c>) の猶予期間は「同期先へ書かれてからの
