...
削除は「直前に読み直した状態のままなら」という条件付きで行う。読み直してから消すまでの間に別の PC が同じ内容を送り直していると、その実体はこれから公開される manifest に参照されるためである。S3 互換モードは `If-Match` で不可分に判定する。同期フォルダには同等の操作が無いため削除の直前に読み直す形になり、そこからの一瞬は残る。
